...
async function checkLock() {
  const s = await NotebookStorage.getSettings();
  if (s.dashboardLocked && s.passwordHash) {
...
function openSettings() {
  NotebookStorage.getSettings().then(s => {
    document.getElementById("toggle-lock").checked    = s.dashboardLocked;
    document.getElementById("toggle-overlay").checked = s.overlayEnabled;
    document.getElementById("password-section").classList.toggle("hidden", !s.dashboardLocked);
  });
...
document.getElementById("settings-bg").addEventListener("click", e => {
  if (e.target === document.getElementById("settings-bg")) closeSettings();
});
...
document.getElementById("toggle-lock").addEventListener("change", async (e) => {
  ...
  if (!e.target.checked) {
    const s = await NotebookStorage.getSettings();
    await NotebookStorage.saveSettings({ ...s, dashboardLocked: false, passwordHash: null });
    toast("Password lock disabled.", "ok");
  }
});

document.getElementById("toggle-overlay").addEventListener("change", async (e) => {
  const s = await NotebookStorage.getSettings();
  await NotebookStorage.saveSettings({ ...s, overlayEnabled: e.target.checked });
  toast(e.target.checked ? "Overlay enabled." : "Overlay disabled.", "ok");
});

document.getElementById("btn-save-password").addEventListener("click", async () => {
  const pw  = document.getElementById("new-password").value;
  const pw2 = document.getElementById("confirm-password").value;
  if (!pw)       return toast("Enter a password.", "err");
  ...
  const s    = await NotebookStorage.getSettings();
  await NotebookStorage.saveSettings({ ...s, dashboardLocked: true, passwordHash: hash });
...
document.getElementById("btn-remove-password").addEventListener("click", async () => {
  if (!confirm("Remove password lock?")) return;
  const s = await NotebookStorage.getSettings();
  await NotebookStorage.saveSettings({ ...s, dashboardLocked: false, passwordHash: null });
